...
uses
 ...
 {$IFDEF FPC}
  ZLib;
 {$ELSE ~FPC}
  System.ZLib;             {interface to access ZLIB C OBJ files} // Added by Rapid D
 {$ENDIF FPC}
...
function deflateInitEx(var strm: TZStream; level: Integer; streamtype: TZStreamType = zsZLib): Integer;     { V8.70 }
function inflateInitEx(var strm: TZStream; streamtype: TZStreamType = zsZLib): Integer;                     { V8.70 }
...
function ZlibCheckInitInflateStream (var strm: TZStream; gzheader: gz_headerp): TZStreamType;
...
type
 ...
 {$IFDEF FPC_TEST2}
   function deflateInit2(var strm: TZStream; level, method, windowBits, memLevel, strategy: cint): cint;
   begin
     Result := deflateInit2_(strm, level, method, windowBits, memLevel, strategy, ZLIB_VERSION, sizeof(TZStream));
   end;

   function inflateInit2(var strm: TZStream; windowBits: cint): cint;
   begin
     Result := inflateInit2_(strm, windowBits, ZLIB_VERSION, sizeof(TZStream));
   end;
 {$ENDIF FPC_TEST2}

...
// By Rapid D
type
  TRDEOFEvent = procedure(Sender: TObject; const Pos : Int64) of object;
...
// Added By Rapid D ---->
    procedure DoUnexpectedEOF(const pos : Int64);
// <-----
  public
    ...
    property OnDebugOut: TsdDebugEvent read FOnDebugOut write FOnDebugOut;
// Added By Rapid D ---->
    property OnUnexpectedEOF: TRDEOFEvent read FOnUnexpectedEOF write FOnUnexpectedEOF;
// <-----
    ...
    function ReadStringUntilChar(AChar: AnsiChar; MustBe : Boolean = false): Utf8String;
...

{
  ...
  public
    ...
    procedure AttributeAdd(const AName, AValue: UnicodeString); overload;
...
procedure TXmlNode.AttributeAdd(const AName, AValue: UnicodeString);
var
  A: TsdAttribute;
begin
  A := TsdAttribute.Create(TNativeXml(FOwner));
  A.Name := UTF8Encode(AName);
  A.Value := UTF8Encode(AValue);
  AttributeAdd(A);
end;
...
begin
  ...
  FNameID := AddString(sdTrim(P.ReadStringUntilChar('=', True), IsTrimmed));
  if assigned(FCoreValue) then
    // value
...
begin
  ...
  if QuoteChar = cQuoteCharStyleNames[qsQuote] then
    ...
    begin
      ...
      if p.FEndOfStream then //Added by Rapid D
        p.DoUnexpectedEOF(p.Position);
...
begin
  ...
  repeat
    ...
    begin
      ...
      p.DoUnexpectedEOF(P.Position); // Added by Rapid D
      exit;
    end;
    P.MoveBack;
    ...
    begin
      ...
      begin
        ...
        // Read end tag
        EndTagName := sdTrim(P.ReadStringUntilChar('>', True), IsTrimmed);
...
begin
  ...
  // Added by Rapid D
    if NodeClosingStyle <> ncNone then
  // <<<<
...
// By Rapid D >>>>>
procedure TsdXmlParser.DoUnexpectedEOF(const pos: Int64);
begin
  if Assigned(FOnUnexpectedEOF) then
    FOnUnexpectedEOF(Self, pos);
end;// <<<<<
...
begin
  ...
  Result := ReadStringUntilChar(AQuote, True);
end;
...
begin
  ...
  DoUnexpectedEOF(GetPosition);
end;

function TsdXmlParser.ReadStringUntilChar(AChar: AnsiChar; MustBe : Boolean = false): Utf8String;
var
  Count: integer;
  StartIdx: integer;
begin

...
// write out translated bytes to the image storage
            ...
            LZWWriteBitmap;
//            if fDataStream.Position < fDataStreamSize then
            if fDataStream.Seek(0, soFromCurrent) < fDataStreamSize then
              pc := LZWGetCode
            else Done := True;    
            rMaxVal := false;
...
with fZipData^ do
    begin
      FillMemory(@rPrefix[0], kGifCodeTableSize * SizeOf(Integer), 0); // By Rapid D
      FillMemory(@rSuffix[0], Length(rSuffix) * SizeOf(Integer), 0); // By Rapid D

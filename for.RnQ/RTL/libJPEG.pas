...
Added some new features provided libJPEG-turbo by Gary_Ch. (11-07-2013)
...
CHANGELOG
  ...
  2013.07.11
    Code formatting
    Added new libJPEG-turbo color spaces
  2013.07.13
    Conditional compilation directives JCS_EXTENSIONS and JCS_ALPHA_EXTENSIONS
}

{ TODO: новые поля в записях, если JPEG_LIB_VERSION >= 70 }
{ TODO: TurboJPEG API (turbojpeg.h) }
{ TODO: поддержка режима 12 бит / канал }
interface
...
type
  ...
  JCOEFBITSPTR = ^JCOEFBITS;                     // Нет в
  JCOEFBITS = array [0..DCTSIZE2 -1] of integer; // jpeglib.h


  { Types for JPEG compression parameters and working tables. }

  // TODO: Mцglicherweise betrifft dies auch die Rьckgabewerte?
  // (Windows-1250)
  // translate.google.ru: Возможно, это также влияет на возвращаемые значения
...
//#endif
  ...
  jpeg_scan_info = record 
    ...
    Ss: integer;  { progressive JPEG spectral selection parms }
    Se: integer;
    Ah: integer;  { progressive JPEG successive approx. parms }
    Al: integer;
  end;
...
type
  ...
  // dummy types (jpeglib.h line 1170 - 1195)
  ...
  jpeg_memory_mgr = record
    ...
    alloc_sarray: function(cinfo: j_common_ptr; pool_id: integer; samplesperrow: JDIMENSION; numrows: JDIMENSION): 
  JSAMPARRAY; cdecl;
    alloc_barray: function(cinfo: j_common_ptr; pool_id: integer; blocksperrow: JDIMENSION; numrows: JDIMENSION): 
  JBLOCKARRAY; cdecl;
    request_virt_sarray: function(cinfo: j_common_ptr; pool_id: integer; pre_zero: boolean; samplesperrow: JDIMENSION; 
  numrows: JDIMENSION; maxaccess: JDIMENSION) : jvirt_sarray_ptr; cdecl;
    request_virt_barray: function(cinfo: j_common_ptr; pool_id: integer; pre_zero: boolean; blocksperrow: JDIMENSION; 
  numrows: JDIMENSION; maxaccess: JDIMENSION): jvirt_barray_ptr; cdecl;
    realize_virt_arrays: procedure(cinfo: j_common_ptr); cdecl;
    access_virt_sarray: function(cinfo: j_common_ptr; ptr: jvirt_sarray_ptr; start_row: JDIMENSION; num_rows: JDIMENSION; 
  writable: boolean): JSAMPARRAY; cdecl;
    access_virt_barray: function(cinfo: j_common_ptr; ptr: jvirt_barray_ptr; start_row: JDIMENSION; num_rows: JDIMENSION; 
  writable: boolean): JBLOCKARRAY; cdecl;
...
//  jpeg_stdio_src: procedure(cinfo: j_decompress_ptr; FILE * infile); cdecl;
  ...
  jpeg_add_quant_table: procedure(cinfo: j_common_ptr; which_tbl: integer; const basic_table: pcardinal; scale_factor:
integer; force_baseline: boolean); cdecl;
  ...
  jpeg_write_marker: procedure(cinfo: j_compress_ptr; marker: integer; const dataptr: JOCTET_ptr; datalen: cardinal); 
cdecl;
  { Same, but piecemeal. }
...
var
  ...
  jpeg_set_marker_processor: procedure(cinfo: j_decompress_ptr; marker_code: integer; routine: jpeg_marker_parser_method); 
cdecl;
...
begin
  ...
  if libJPEG_RefCount <= 0 then begin
    if libJPEG_Handle <> {$ifdef win32} 0 {$else} {$ifdef win64} 0 {$else} nil {$endif} {$endif} then begin
      {$ifdef win32}

...
//  AsyncCalls,
 {$ENDIF}
  Protocols_all,
  TLGConsts, Protocol_TLG, TLG, //TLGCodes,
  menusUnit;
...
//   contact.xStatusStr := xstatusBox.Text;
 ...
 TTLGcontact(contact).ssImportant := ssNoteStrEdit.Text;
 TTLGcontact(contact).ssCell  := CellularEdt.Text;
  TTLGcontact(contact).ssCell2 := CellularEdt2.text;
  TTLGcontact(contact).ssCell3 := CellularEdt3.text;
 TTLGcontact(contact).ssMail := localMailEdt.Text;
...
//  aboutBox.text := unUTF(about);
  ...
  if contact.isMyAcc then
    ipBox.text := TTLGSession(contact.Proto).getLocalIPstr
  else
...
//  i := interests.Count;
  AllInfoMemo.Text := UTF8ToStr(allInfo);
 ...
 {$IFDEF UseNotSSI}
            TTLGSession(contact.fProto).useSSI and
 {$ENDIF UseNotSSI}
            contact.Proto.isOnline;
  publicChk.checked := TTLGSession(contact.Proto).pPublicEmail;
...
//  Image1.Picture.Assign(contact.icon);
  LUPDDATEEdt.Text  := IntToHex(Integer(TTLGcontact(contact).lastUpdate_dw), 8);
  LInfoDATEEdt.Text := IntToHex(TTLGcontact(contact).lastinfoupdate_dw, 8);
...
//    loginMailEdt.Text := Attached_login_email;
   ...
   begin
    protoBox.text := ifThen(TTLGcontact(contact).protoV=0, getTranslation(Str_unk),  'ver.'+intToStr(TTLGcontact(contact).protoV));
    loginMailEdt.Text := '';
...
begin
//  TTLGSession(contact.fProto).RequestXStatus(contact.uid);
end;
...
//     TTLGSession(contact.Proto).sendQueryInfo(contact.UID2cmp);
//     wpS.token := TTLGcontact(contact).InfoToken;
//     TTLGSession(contact.fProto).sendWPsearch2(wpS, 0, False);
    end
end;
...
//  MarStsBox.Items.Text := MarStsToStr;
  if TTLGcontact(contact).infoUpdatedTo = 0 then
    TTLGSession(contact.Proto).sendQueryInfo(contact.UID2cmp);
...
//  avtSaveBtn.Enabled := itsme;
  ...
  avtTS.TabVisible := TTLGSession(contact.Proto).AvatarsSupport;
  ClrAvtBtn.Enabled := (itsme and (TTLGSession(contact.Proto).myAvatarHash > '')) or
     (not itsme and (contact.Icon.Hash_safe > '')and(TTLGcontact(contact).IconID = ''));
...
if not contact.isMyAcc then
  ...
  if OnlFeature(contact.Proto) then
    begin
//    c := Tcontact.create(0);
    c := TTLGcontact(contact.Proto.getMyInfo);
    c.uid := contact.uid;
    c.nick := nickBox.text;
...
//    c.xStatusStr := xstatusBox.Text;
    TTLGSession(c.Proto).pPublicEmail := publicChk.checked;
...
//    TTLGSession(c.fProto).sendsaveMyInfoNew(c);
...
begin
 ...
 s := AccPath + avtPath + contact.getFN + '.photo.jpeg';
  ...
  contactPhoto := NIL;
 SetPhtBtnEnb(False);
 if LoadFromURL(Format(ICQ_PHOTO_AVATAR, [contact.UID2cmp, TTLGcontact(contact).gender]), s, 0, True) then
    if Assigned(self) then //and Assigned(PhotoImg)and Assigned(PhotoImg.Picture) then
      begin
        loadPic2(s, contactPhoto);
        if Assigned(self) and Assigned(PhotoPBox) then
...
begin
//  TTLGSession(contact.fProto).sendInfoStatus(StsMsgEdit.Text);
...
begin
  if not Assigned(serverSSI.items) then
    TTLGSession(contact.fProto).RequestContactList(false)
   else
    begin
      i := FindSSIItemType(serverSSI, FEEDBAG_CLASS_ID_PDINFO);
      if i >= 0 then
       with TOSSIItem(serverSSI.items.Objects[i]) do
        TTLGSession(contact.fProto).SSI_DeleteItem(GroupID, ItemID, ItemType);
...
function TviewinfoFrm.isUpToDate: boolean;
  ...
  begin
    result := (d>10000) and (d<now) and (d>TTLGcontact(contact).infoUpdatedTo)
  end;
...
begin
//  TTLGSession(contact.fProto).getUINStatus(contact.UID2cmp);
//  getUINStatus
//  contact
end;
...
//    TTLGSession(contact.fProto).SSI_UpdateContact(TTLGcontact(contact));
end;
...
begin
  ...
  if itsme then
   begin
{
    if TTLGSession(contact.fProto).SSI_deleteAvatar then
     begin
      TTLGSession(contact.fProto).myAvatarHash := '';
//      updateAvatarFor(contact);
     end;
}
   end;
...
begin
  ...
  //      frm.AvtImg.Picture.Assign(cnt.icon);
        avtLoadBtn.Enabled := TTLGcontact(contact).IconID > '';
...
begin
  if contact is TTLGcontact then
    if TTLGcontact(contact).GMTavailable then
      timeBox.text := formatDatetime(timeformat.clock, now-GMToffset0+TTLGcontact(contact).getGMT);
end;

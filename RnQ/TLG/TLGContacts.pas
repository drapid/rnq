...
//              InterestBlock: array of TinterestBlock;
  ...
  TTLGContact = class(TRnQContact)
  public
    TLG_ID: String; // Integer?
//    UININT: Integer;
    NoClient: Boolean;
    Official: Boolean;
    ClientClosed: TDateTime;
    Status: TTLGStatus;
    PrevStatus: TTLGStatus;
    InvisibleState: Byte;
    UserType: TTLGContactType;
    Crypt: record
      supportEcc: Boolean;
...
//  Tcontact = TTLGContact;
//  function  ICQCL_buinlist(cl: TRnQCList; Proto: IRnQProtocol): string;
  procedure ICQCL_setStatus(cl: TRnQCList; st: TTLGStatus);
...
uses
  ...
  TLG, viewTLGinfoDlg, Protocol_TLG;

///////////////////////////////////////////////////////////////////////////

constructor TTLGContact.create(pProto: TRnQProtocol; const uin_: TUID);
begin
...
destructor TTLGContact.Destroy;
begin
...
procedure TTLGContact.SetOffline;
begin
  OfflineClear;
  status := TLGConsts.SC_OFFLINE;
end;

procedure TTLGContact.OfflineClear;
begin
  invisibleState := 0;
...
function TTLGContact.getGMT: TdateTime;
begin result := -GMThalfs/48 end;

function TTLGContact.GMTavailable: boolean;
begin result := abs(GMThalfs) < 100 end;

function TTLGContact.isOnline: Boolean;
begin
  result := not (status in [TLGConsts.SC_OFFLINE, TLGConsts.SC_UNK])
end;

function TTLGContact.isAcceptFile: Boolean;
begin
  Result := False;
end;

function TTLGContact.isInvisible: Boolean;
begin
  Result := InvisibleState > 0;
end;

function TTLGContact.isOffline: Boolean;
begin
  result := status = TLGConsts.SC_OFFLINE;
end;

function TTLGContact.canEdit: Boolean;
begin
...
procedure TTLGContact.SetDisplay(const s: String);
var
  s0: String;
begin
  s0 := Display;
  ...
  if s0 <> Display then
    TTLGSession(fProto).SSI_UpdateContact(self);
end;

procedure TTLGContact.ViewInfo;
var
  vi: TRnQViewInfoForm;
begin
...
function TTLGContact.uinAsStr: String;
begin result := uid end;

function TTLGContact.uin2Show: String;
var
  i, m, n, l: byte;
  s: String;
begin
...
//     s := UnDelimiter(uid);
     l := length(s);
     if (l = 12) and (s[1]='+') then
      begin
       result := Copy(s, 1, 2) + ' (';
       result := result + Copy(s, 3, 3) + ') ';
       result := result + Copy(s, 6, 3) + '-';
       result := result + Copy(s, 9, 2) + '-';
       result := result + Copy(s, 11, l-10+1);
...
function TTLGContact.getStatusName: String;
var
  s1: String;
begin
  s1 := '';
...
//   end;
  if (s1 > '') then
    if status <> TLGConsts.SC_ONLINE then
...
function TTLGContact.statusImg: TPicName;
begin
...
function TTLGContact.getStatus: byte;
begin
  result := byte(status);
end;

class function TTLGContact.trimUID(const sUID: TUID): TUID;
const
  validDelemiters = ['(', ')', ' ', '-'];
var
  i: Word;
...
//  SetLength(Result, length(sUID));
  ...
  if Length(s1) = 0 then
    Exit;
  t := 0;
  if s1.StartsWith(TTLGSession._GetProtoName + '_') then
    s2 := TUID(copy(s1, length(TTLGSession._GetProtoName)+2, 250))
   else
    s2 := s1;
      for I := 1 to length(sUID) do
       begin
        ch := s2[i];
        if not (ch in validDelemiters) then
//        if (ch = '+') or ch.IsDigit then
         begin
          inc(t);
          if i <> t then
            s2[t] := ch;
...
function TTLGContact.GetDBrow: RawByteString;
...
function TTLGContact.ParseDBrow(ItemType: Integer; const item: RawByteString): Boolean;
...
//    DBFK_ssNickname: self.ssNickname := UnUTF(item);
    ...
    DBFK_UTYPE: self.UserType := TTLGContactType(str2int(item));
  else
    Result := inherited ParseDBrow(ItemType, item);
  end;
end;

{operator TTLGContact.Implicit(a: AnsiString): TContact; // Implicit conversion of an Integer to type TMyClass
begin
  result := TTLGContact.create(a);
end;
}
///////////////////////////////////////////////////////////////////
...
//  max:=count-1;
  repeat
   inc(result);
   c := cl.getAt(result);
   if Assigned(c)and (c is TTLGContact) then
    if TTLGContact(c).SSIID = ssid then
     break;
...
begin
  result := '';
  for i:=0 to cl.count-1 do
//   with TTLGContact(cl.items[i]) do
   with TTLGContact(cl.getAt(i)) do

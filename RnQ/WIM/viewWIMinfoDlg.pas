...
//   contact.xStatusStr := xstatusBox.Text;
 ...
 TWIMcontact(contact).ssImportant := ssNoteStrEdit.Text;
 TWIMContact(contact).ssCell  := CellularEdt.Text;
  TWIMContact(contact).ssCell2 := CellularEdt2.text;
  TWIMContact(contact).ssCell3 := CellularEdt3.text;
 TWIMContact(contact).ssMail := localMailEdt.Text;
...
//  Image1.Picture.Assign(contact.icon);
  LUPDDATEEdt.Text  := IntToHex(Integer(TWIMContact(contact).lastUpdate_dw), 8);
  LInfoDATEEdt.Text := IntToHex(TWIMContact(contact).lastinfoupdate_dw, 8);
...
begin
//  TWIMSession(contact.fProto).RequestXStatus(contact.uid);
end;
...
begin
  CliCapsMemo.Clear;
  for i in  TWIMContact(contact).capabilitiesSm do
    if HEXCapsChk.Checked then
      addHex(CAPS_sm2big(i))
     else
      CliCapsMemo.Lines.Append(getTranslation(CapsSmall[i].s));

  for i in  TWIMContact(contact).capabilitiesBig do
    if HEXCapsChk.Checked then
      addHex(BigCapability[i].v)
     else
       CliCapsMemo.Lines.Append(getTranslation(BigCapability[i].s));

  for i in  TWIMContact(contact).capabilitiesXTraz do
    if HEXCapsChk.Checked then
      addHex(XStatusArray[i].pidOld)
     else
       CliCapsMemo.Lines.Append(getTranslation('Old XStatus "%s"', [getTranslation(XStatusArray[i].Caption)]));

  if length( TWIMContact(contact).extracapabilities) > 15 then
    for I := 0 to length( TWIMContact(contact).extracapabilities) div 16 - 1 do
      addHex(copy( TWIMContact(contact).extracapabilities, i*16+1, 16));
end;
...
begin
 ...
 s := AccPath + avtPath + contact.getFN + '.photo.jpeg';
  ...
  contactPhoto := NIL;
 SetPhtBtnEnb(False);
 if LoadFromURL(Format(ICQ_PHOTO_AVATAR, [contact.UID2cmp, TWIMContact(contact).gender]), s, 0, True) then
    if Assigned(self) then //and Assigned(PhotoImg)and Assigned(PhotoImg.Picture) then
      begin
        loadPic2(s, contactPhoto);
        if Assigned(self) and Assigned(PhotoPBox) then
...
begin
//  TWIMSession(contact.fProto).sendInfoStatus(StsMsgEdit.Text);
...
begin
  if not Assigned(serverSSI.items) then
    TWIMSession(contact.fProto).RequestContactList(false)
   else
    begin
      i := FindSSIItemType(serverSSI, FEEDBAG_CLASS_ID_PDINFO);
      if i >= 0 then
       with TOSSIItem(serverSSI.items.Objects[i]) do
        TWIMSession(contact.fProto).SSI_DeleteItem(GroupID, ItemID, ItemType);
...
function TviewinfoFrm.isUpToDate: boolean;
  ...
  begin
    result := (d>10000) and (d<now) and (d>TWIMContact(contact).infoUpdatedTo)
  end;
...
begin
//  TWIMSession(contact.fProto).getUINStatus(contact.UID2cmp);
//  getUINStatus
//  contact
end;
...
//    TWIMSession(contact.fProto).SSI_UpdateContact(TWIMContact(contact));
end;
...
begin
  ...
  if itsme then
   begin
{
    if TWIMSession(contact.fProto).SSI_deleteAvatar then
     begin
      TWIMSession(contact.fProto).myAvatarHash := '';
//      updateAvatarFor(contact);
     end;
}
   end;
...
begin
  ...
  //      frm.AvtImg.Picture.Assign(cnt.icon);
        avtLoadBtn.Enabled := TWIMContact(contact).IconID > '';
...
begin
  if contact is TWIMContact then
    if TWIMContact(contact).GMTavailable then
      timeBox.text := formatDatetime(timeformat.clock, now-GMToffset0+TWIMContact(contact).getGMT);
end;

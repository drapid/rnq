...
uses
  ...
  WIM, Protocol_WIM, WIMConsts, RnQProtocol, //contacts,
...
procedure TWIMFr.unInitPage;
begin
  clearMenu(CapsPpp.Items);
end;

procedure TWIMFr.AddCapsBtnClick(Sender: TObject);
begin
...
procedure TWIMFr.onCapsMenuClick(Sender: TObject);
var
  s : RawByteString;
begin
//  with TRnQButton(Sender).BoundsRect do
...
procedure TWIMFr.CapEditChange(Sender: TObject);
begin
...
procedure TWIMFr.AddCapsChkClick(Sender: TObject);
begin
  updateVisPage;
end;

procedure TWIMFr.applyPage;
var
  mustUpdPerms,
  mustUpdPerms2,
  mustUpdSts,
...
begin
  if not Assigned(Account.AccProto)
    ...
    Exit;
  fICQ := TWIMSession(Account.AccProto.ProtoElem);
...
//     if not showInvisSts and (fICQ.visibility in [VI_invisible, VI_privacy]) then
     if not showInvisSts and (fICQ.visibility in [VI_invisible]) then
      begin
...
procedure TWIMFr.updateVisPage;
begin
...
procedure TWIMFr.SpeedButton1Click(Sender: TObject);
begin
 openURL('http://wwp.icq.com/scripts/online.dll?icq='+
          Account.AccProto.getMyInfo.UID2cmp+'&img=3')
end;

procedure TWIMFr.UpdVis(Sender: TObject);
begin
  updateVisPage;
end;

procedure TWIMFr.UpdVis1(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  updateVisPage;
end;

procedure TWIMFr.SBSelInvisClClick(Sender: TObject);
begin
...
procedure TWIMFr.CloseAction(sender:Tobject);
var
  wnd: TselectCntsFrm;
//  cl: TcontactList;
//  msg: string;

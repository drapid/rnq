...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Types, StdCtrls,
  ExtCtrls,
  RnQGraphics32, RnQButtons, AwImageGrid,
...
type
  TFEmoji = class(TForm)
    ...
    function GetEmoji(num: Integer): TRnQBitmap;
...
//  procedure ShowEmojiMenu(rnqcon: TRnQContact; t: tpoint);
...
//  FEmoji: TFEmoji;
  ...
  EmojiSprite: TRnQBitmap;

implementation

uses
  RnQNet, RnQProtocol, utilLib,
  RDGlobal, events, history,
   chatDlg, RnQLangs, RnQGlobal, globalLib, RQUtil, RQThemes;
...
const
  ...
  emojiCodePoints: array [0..1275] of array [1..2] of Integer = (
...
procedure TFEmoji.CreateParams(var Params: TCreateParams);
begin
...
procedure TFEmoji.FormShow(Sender: TObject);
begin
  showEmojiExt(openedExt);
end;

procedure TFEmoji.UpdTmrTimer(Sender: TObject);
begin
...
procedure TFEmoji.PutEmoji(FExt, FEmoji: Integer);
var
...
{  if not png.Empty then
  begin
    ...
    begin
      cp := TCharacter.ConvertFromUtf32(emojiCodePoints[FEmoji][1]);
      if not (emojiCodePoints[FEmoji][2] = 0) then
        cp := cp + TCharacter.ConvertFromUtf32(emojiCodePoints[FEmoji][2]);
...
procedure TFEmoji.showEmojiExt(ext: Integer);
var
  emojiGrid: TAwImageGrid;
  num: Integer;
begin
  ...
  begin
    emojiGrid := TAwImageGrid.Create(Self);
    with emojiGrid do
    begin
      Width := 0;
      Height := 0;
      ...
      CellLayout := tlCenter;
      CellHeight := emojiSize + 15;
      CellWidth := emojiSize + 15;
      CellSpacing := 2;
      Color := clBtnFace;
      WheelScrollLines := 1;
      Sorted := False;
...
procedure TFEmoji.InvalidateEmoji(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TFEmoji.SendSelectedEmoji(Sender: TCustomImageGrid; Index: Integer);
begin
  Self.Hide;
  GoToChat;
...
function TFEmoji.GetOffset(num: Integer): TPoint;
var
...
function TFEmoji.GetEmoji(num: Integer): TRnQBitmap;
var
  line: Integer;
  pt: TPoint;
  hasAlpha: Boolean;
  bpp: Integer;
begin
{  Result := TPNGImage.CreateBlank(EmojiSprite.Header.ColorType, EmojiSprite.Header.BitDepth, emojiSize, emojiSize);
  OutputDebugString(PChar(inttostr(bpp)));
  pt := GetOffset(num);
  hasAlpha := EmojiSprite.Header.ColorType in [COLOR_GRAYSCALEALPHA, COLOR_RGBALPHA];
  for line := 0 to Result.Height - 1 do
  begin
    if hasAlpha then
      CopyMemory(Result.AlphaScanline[line], Ptr(NativeUInt(EmojiSprite.AlphaScanline[line + pt.Y]) + NativeUInt(pt.X)), Result.Width);
    CopyMemory(Result.Scanline[line], Ptr(NativeUInt(EmojiSprite.Scanline[line + pt.Y]) + NativeUInt(pt.X * 3)), Result.Width * 3);
  end;
}
end;

procedure TFEmoji.CreateExtBtns();
var
  i: Integer;
...
procedure TFEmoji.FormCreate(Sender: TObject);
begin
...
procedure TFEmoji.OnExtBtnClick(Sender: TObject);
begin
  showEmojiExt((Sender as TRnqSpeedButton).Tag);
  RefreshExtBtnStates;
end;

procedure TFEmoji.RefreshExtBtnStates;
var
  i: Integer;
  btn: TRnQSpeedButton;
begin
...
procedure TFEmoji.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
  Index: Integer;
begin
  case key of
...
procedure TFEmoji.FormPaint(Sender: TObject);
var
  DC: HDC;
  Rgn: HRGN;
  brF : HBRUSH;
...
//  rnqContact := rnqcon;
{
  ...
  if not Assigned(FEmoji) then
    FEmoji := TFEmoji.Create(nil);
  FEmoji.Height := (emojiSize + 10 + 10) * 5 + FEmoji.exts.Height;
  FEmoji.Width := (emojiSize + 10 + 6) * emojiContents.Count;

  scr := Screen.MonitorFromPoint(t).WorkareaRect;
  ar[1] := Rect(t.X, t.Y - FEmoji.Height, t.X + FEmoji.Width, t.Y);
  ar[2] := Rect(t.X - FEmoji.Width, t.Y - FEmoji.Height, t.X, t.Y);
  ar[3] := Rect(t.X, t.Y, t.X + FEmoji.Width, t.Y + FEmoji.Height);
  ar[4] := Rect(t.X - FEmoji.Width, t.Y, t.X, t.Y + FEmoji.Height);
  a := Rect(0, 0, 0, 0);
